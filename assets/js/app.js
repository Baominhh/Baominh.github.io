...
function noti(status, message, delay = 1500) {
    ...
    if (typeof $(".toast-container").html() === "undefined") {
        ...
        $(".toast-container").append(
            `<div role="alert" aria-live="assertive" toasts-id="${date}" data-bs-delay="${delay}" aria-atomic="true" class="toast fade show"><div class="toast-header bg-${statuss}"><h5 class="me-auto m-0 fs-14">${icon} ThĂ´ng bĂ¡o</h5> <span data-bs-dismiss="toast" aria-label="Close" class="text-white cursor-pointer"><i class="fa fa-times fw-normal"></i></span></div><div class="toast-body"><span>${message}</span></div></div></div>`
        );
...
function copy(text) {
    ...
    noti('success', `ÄĂ£ sao chĂ©p <b>${text}</b>`)
}
$(function () {
